  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (hasSeen) {
        ...
        const t3 = setTimeout(() => setIntroPhase(3), 5500); 
        const t4 = setTimeout(() => setIntroPhase(4), 7500); 
        const t5 = setTimeout(() => setIntroPhase(5), 9500); 
        const t6 = setTimeout(() => {
          setShowStats(true);
    ...
    <div className="fade-in" style={{ animation: 'fadeIn 2s' }}>
      ...
      <div className="main-grid">
        ...
        <div className="dialogue-box" style={{ flexDirection: 'column', gap: '16px' }}>
          ...
          >
            <span style={{color: '#888'}}>ATTACK:</span> {animatedStats.atk} (Full Stack)
          </div>
          <div 
            style={{ cursor: 'pointer', userSelect: 'none' }} 
            onClick={(e) => handleStatClick(e, animatedStats.def)}
          >
            <span style={{color: '#888'}}>DEFENSE:</span> {animatedStats.def} (Systems Security)
          </div>
          <div 
            style={{ cursor: 'pointer', userSelect: 'none' }} 
            onClick={(e) => handleStatClick(e, animatedStats.spd)}
          >
            <span style={{color: '#888'}}>SPEED:</span> {animatedStats.spd} (Rapid Prototyping)
          </div>
          <div style={{marginTop: '16px'}}>
            <span style={{color: '#888'}}>EXP:</span> 2+ YEARS<br/>
            <div className="tooltip" onClick={(e) => handleStatClick(e, 8)}>
              <span style={{color: '#888'}}>GOLD:</span> 8+ PROJECTS
